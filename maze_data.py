...
class MazeBase(object):
    ...
    def deny_ghosts_access(self, ghosts, nodes):
        nodes.deny_acc_list(*(self.add_offset(2, 3) + (LEFT, ghosts)))
        nodes.deny_acc_list(*(self.add_offset(2, 3) + (RIGHT, ghosts)))

        for direction in list(self.ghost_node_deny.keys()):
            for values in self.ghost_node_deny[direction]:
                nodes.deny_acc_list(*(values + (direction, ghosts)))

class FirstMaze(MazeBase):
    def __init__(self):
...
class SecondMaze(MazeBase):
    def __init__(self):
        MazeBase.__init__(self)
        self.name = "SecondMaze"
...
class MazeData(object):
    def __init__(self):
        self.obj = None
        self.maze_dict = {0:FirstMaze, 1:SecondMaze}
